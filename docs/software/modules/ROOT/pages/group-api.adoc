...
include::partial$group-privileges.adoc[]
...
|`displayname`|__String__. A unique display name string for the user group, for example, `Analysts`. 
...
* `BYPASSRLS`
|`grouptype` +
__Optional__ a|__String__. Type of user group. Default value is `LOCAL_GROUP`, which indicates that the user group is created locally in the ThoughtSpot system.    
|`tenantid` +
__Optional__|__String__. GUID of the tenant for which the user group is being created.
|`visibility` +
__Optional__|__String__. Visibility of the user group. The `visibility` attribute is set to `DEFAULT`. The `DEFAULT` attribute makes the user group visible for other user groups and allows them to share  objects.  
|===
...
[NOTE]
====
ThoughtSpot also has a default group called `ALL`. When you create new users in ThoughtSpot, they are automatically assigned to the `ALL` group. By default, the members of `ALL` do not have permissions to download or upload data. You can use this API to add these privileges to the `ALL` group.
====
...
* `A3ANALYSIS`
* `BYPASSRLS`
|`groupNames`|__String__. A JSON array of group names to which you want to add the privilege. To add a privilege to `ALL` group, specify `ALL`.
|====
...
|`groupNames`|__String__. A JSON array of group names from which you want to remove the privilege. To remove a privilege from `ALL`, specify `ALL`.
|====
