...
|`principalids`|__String__. A JSON array of the GUID of the users with whom you want to share the visualization.
...
|`notify` +
__Optional__|__Boolean__. When set to `true`, a notification is sent to the user after the visualization is shared.
|`message` +
...
----
|`dependentshare` __Optional__|__Boolean__. Object permission details for the dependent objects. When set to `true`, the API returns the permission details for the dependent objects for the specified object types and IDs.
|====
