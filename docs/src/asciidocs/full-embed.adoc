...
[source,javascript]
----
import { AppEmbed, Page, AuthType, init, prefetch, EmbedEvent } from '@thoughtspot/visual-embed-sdk';
----
...
For example, to disable the *Download* action in the *More* (*...*) menu for visualizations and answers, specify `Action.Download` in the `disabledActions` attribute.
...
disableProfileAndHelp _optional_::
...
_Array of strings_. The actions to hide on a pinboard, visualization, or answer pages in the embedded UI.
+
...
+
++++
<a href="{{previewPrefix}}/playground/fullApp?pageId=fullApp" id="preview-in-playground" target="_blank">Preview in Playground</a>
++++


runtimeFilters [small]_optional_::
Runtime filters to apply when a pinboard or visualization page loads.
Runtime filters provide the ability to pass filter specifications as URL query parameters and filter data at the time of retrieval.
+
