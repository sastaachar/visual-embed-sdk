...
Guide::
The *Guide* menu displays the _ThoughtSpot Developer Documentation_. The Developer documentation provides step-by-step instructions for embedding ThoughtSpot in your application using Visual Embed SDK and REST APIs.
...
Provides security controls to add third-party domains as xref:security-settings.adoc[trusted hosts for CORS and CSP]. The *Security Settings* page also allows you to xref:trusted-authentication.adoc[enable trusted authentication], or xref:configure-saml.adoc[add a SAML redirect domain] to the allowed list of domains.
