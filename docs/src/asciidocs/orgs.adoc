...
|REST API v2.0 endpoints | [tag greenBackground]#✓# | [tag greenBackground]#✓#
|=====
...
If trusted authentication is enabled on your instance, you can use the `secret key` to generate an authentication token for your Org context using one of the following REST API endpoints:

* REST API v1 - xref:session-api.adoc#session-authToken[`/tspublic/v1/session/auth/token`]
* REST API v2.0 - xref:rest-api-v2-reference.adoc#_authentication[`/api/rest/2.0/auth/token/full`] or xref:rest-api-v2-reference.adoc#_authentication[`/api/rest/2.0/auth/token/object`]

If the user doesn't exist in the ThoughtSpot system, you can create a user account just-in-time and dynamically assign privileges by adding the user to `groups`.

=== SAML authentication

For SAML authentication, ensure that the Org support is enabled for SAML authentication.
