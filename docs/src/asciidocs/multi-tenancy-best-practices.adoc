...
Just like in physical housing, tenancy within software system takes many different physical and logical forms. Within this section, a _multi-tenanted_ system is one where an administrator see everything at once from a single login, while a _single-tenanted_ system does have an _all content_ view available, even to an administrator account.

== Multi-tenancy in ThoughtSpot using groups
...
There are two aspects of groups which interact to create the _wall_ between customer organizations: “shared content” and the “sharing visibility” property of groups and users.
...
If there is a separate database connection for each customer organization, then the database pattern is single-tenant, and you will need to create objects for each tenant. ThoughtSpot provides APIs and the TML document language for automating the creation or deployment of each tenant’s objects.
...
RLS can be considerably more complex than just splitting at the tenant level and ThoughtSpot does facilitate these more complex models. For a detailed description, see the link:https://community.thoughtspot.com/s/article/How-to-secure-your-data-in-ThoughtSpot-Examples-and-Best-Practices[comprehensive examples and best practices guide, window=_blank]). However, the basics of RLS to split at the tenant key level are always present and require the creation of the RLS groups.
...
The _multi-tenant database model_ is designed on the following principles:
