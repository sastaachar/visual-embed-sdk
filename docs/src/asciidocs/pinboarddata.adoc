...
|`id`|__String__. GUID of pinboard.
|`vizid` __Optional__|__String__. A JSON array of GUIDs of the visualizations. 
|`batchsize`|__Integer__. The batch size for loading pinboard objects. The system default is -1.
|`pagenumber`|__Integer__. The system default is -1.
|`offset`|__Integer__. The system default is -1. Alternately, set the offset using the following code:
