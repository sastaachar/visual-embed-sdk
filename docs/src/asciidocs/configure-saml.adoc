...
If you select *no*, the SAML users will not be added in ThoughtSpot upon first successful SSO login. Instead, you must  link:https://docs.thoughtspot.com/software/latest/user-management.html[add users manually, window=_blank].
+
. Click *Save*.

[NOTE]
====
You can also configure SAML authentication support on ThoughtSpot using the `tscli` command `tscli saml configure`. For more information, see link:https://docs.thoughtspot.com/software/latest/saml.html#saml-configure-tscli[Configure SAML using tscli].
====
