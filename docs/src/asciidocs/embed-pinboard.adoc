...
+
By default, the new Liveboard experience is turned off on embedded instances. You can enable it using the `liveboardV2` setting in the `LiveboardEmbed` SDK. However, ThoughtSpot recommends using Liveboards in classic experience mode on embedded apps until the new Liveboard experience is GA.
