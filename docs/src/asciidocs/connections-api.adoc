...
To create or modify data connections through REST APIs, you need to provide a map of JSON attributes as metadata. These attributes vary based on the type of connection.
...
* `name` 
+
__String__. Name of the connection.
* `description`
+
__String__. A short description of the connection.
...
* `role`
+
_String_. Role or the group privilege assigned to your user account in ThoughtSpot. For example, DEVELOPER.
* `warehouse`
+
...
* `schema`
+
_String_.The schema of the tables associated with the database.

////
* `state`

+
__String__. A JSON map of The OAuth client ID and OAuth client secret to authenticate to your Snowflake account. You can use either your service account credentials or the OAuth authentication to connect your data sources.
////
...
--
.Google BigQuery connection attributes
...
* `accountName`
+
__String__. The account name associated with the BigQuery project.
...
--
.Azure Synapse connection attributes
...
--
.Teradata connection attributes
...
--
.Oracle ADW connection attributes
...
* `accountName`
+
_String_. The Net Service Name user name of your Oracle ADW account.
...
--
.Starburst connection attributes
...
--
.SAP HANA connection attributes
...
|`state`|_Integer_. Indicates the OAuth authentication setup.
|====
...
The following example shows the metadata JSON map for creating a Snowflake data connection:
