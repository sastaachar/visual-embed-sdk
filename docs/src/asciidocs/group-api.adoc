...
|`displayname`|__String__. A unique display name string for the user group, for example, `Analyst`.
...
|`visibility` __Optional__|__String__. Visibility of the user group. The `visibility` attribute is set to `DEFAULT`. The `DEFAULT` attribute makes the user group visible to other user groups and allows them to share  objects.
|===
...
|`groupNames`|__String__. A JSON array of group names from which you want to remove the privilege. To remove a privilege from the `All` group, specify `All`.
|====
