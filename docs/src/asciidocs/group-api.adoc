...
If you have admin privileges, you can programmatically modify the properties of a group using the `/tspublic/v1/group/{groupid}` API endpoint. Using this API, you can also assign privileges and modify group visibility.
...
==== Example JSON for group updates
...
If the user group update operation is successful, the API returns the following response code:
...
To add or update the list of users assigned to a group, send a `PUT` request to the `/tspublic/v1/group/{groupid}/users`.
...
|`groupid`|__String__. The GUID of the group from which you want remove the group association. 
|===
...
|===
|Form Parameter|Description
|`userids`|__String__. The GUID of the user that you want to remove from the specified `groupids`.
|`groupids`|__String__. The GUID of the user group from which you want to remove the sepcified `userids`.
|===
