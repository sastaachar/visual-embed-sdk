...
|`import_objects` a|__Array of strings__. An `x-www-form-urlencoded` string containing a JSON array of TML objects to upload, in YAML or JSON format.
...
|`force_create`|__Boolean__. Specifies if you are updating or creating objects. To create new objects, specify `true`.
...
|====
|Form parameter|Description|Default
|`export_ids` a|__Array of strings__. JSON array of the IDs of objects to export. An `x-www-form-urlencoded` string containing a JSON array of ids of objects to export. You receive results in the order you request them.
...
|`formattype` |__String__. The format in which to export the objects. Valid values are `JSON` and `YAML`.|None
