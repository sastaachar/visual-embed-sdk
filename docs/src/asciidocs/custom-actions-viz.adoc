...
To view the *Local* custom actions available on your cluster, create a new chart or table from search, or open a visualization from a pinboard or saved answer, and then click the *Custom actions* icon image:./images/custom-action-icon.png[the Custom actions menu]. The *Custom actions* panel allows you to add, edit, and hide the actions. 
