...
=== GUIDs in TML files
Every object in a ThoughtSpot instance has a unique idenitifer called a GUID. 
...
The process of publishing to different *environments* in ThoughtSpot involves *replacing GUID references* in TML files from the originating environment with the GUIDs of the destination environment. The first time an object is published in a new environment, the new GUID should be recorded in a *mapping file* with the GUID from the originating environment, so that it can be swapped in for any subsequent updates to that object.
