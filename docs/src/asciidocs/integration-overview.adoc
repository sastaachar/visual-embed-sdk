...
In the Trusted authentication method, ThoughtSpot provides a token for users to authenticate and log in. This token is used for obtaining trust from the client applications before granting access to ThoughtSpot content.
+
