...
Once the user has selected or click on an item, the GUID must pass to the appropriate parameter in the options object of the Embed constructor: 

 - `pinboardId` for a xref:embed-pinboard.adoc[whole pinboard] in a PinboardEmbed object
 - `pinboardId` and `vizId` for an xref:embed-a-viz.adoc[individual visualization] on a pinboard through the PinboardEmbed object
