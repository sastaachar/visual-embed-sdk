...
If the API request is successful, ThoughtSpot returns the response header for `set-cookie`. REST  clients can use the cookies from the response to make subsequent REST calls in a non-browser environment.

The API request also returns the application details in the response body.
