...
Note that you sometimes want to display multiple charts or tables that have been saved in a pinboard.  In these situations you should try to embed a single pinboard rather than multiple visualizations for performance reasons.  If you have to embed multiple visualizations, you should xref:prefetch-and-cache.adoc[prefetch the static resources] before displaying the page.  You should also limit the page to fewer than four visualizations.
