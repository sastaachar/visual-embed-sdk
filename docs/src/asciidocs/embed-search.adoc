...
frameParams::
Sets the `width` and `height` dimensions to render the search object in the web browser. You can set the `width` and `height` attribute values in pixels or as a percentage.
