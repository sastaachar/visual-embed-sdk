...
|`fromEpoch` __Optional__|Query|__Epoch time string__. The EPOCH time in milliseconds to set the start time for streaming logs. For example, to set the timestamp as June 1, 2021 8 am, specify `1622534400000`. 
|`toEpoch`  __Optional__|Query|__Epoch time string__. The EPOCH time in milliseconds to set the end time for streaming logs. To set the timestamp as June 2, 2021, 8 am, specify `1622620800000`.
|====

[NOTE]
====
The API allows querying logs for a 24-hour duration only. Make sure the log duration specified in your API request doesn't exceed 24 hours. If you must fetch logs for a longer time range, you can make multiple sequential API requests with different start and end time values. 
====
