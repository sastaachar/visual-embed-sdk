...
* In the REST API SDK [beta blueBackground]^BETA^, the string fields with enumerations, such as the `type` field in metadata API, are changed to class names. If you are using the REST API SDK in your environments, make sure the class names are updated.
