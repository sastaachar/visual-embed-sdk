...
* Note the change in HTTP request method for the following REST v2 API endpoints [beta orangeBackground]^BETA^.  `PUT`.
...
* In the REST API SDK [beta orangeBackground]^BETA^, the string fields with enumerations, such as the `type` field in metadata API, are changed to class names. If you are using the REST API SDK in your environments, make sure the class names are updated. 
...
* The `x-requested-by` header is not mandatory in REST v2 [beta orangeBackground]^BETA^ API calls.
* The `createdBy` attribute in the `/tspublic/rest/v2/metadata/header/search` API will be renamed to `author`.
* Some attributes with the `boolean` data type in the API endpoints will be changed to `string` data type. However, the existing values assigned to these attributes will persist after an upgrade.
|====
