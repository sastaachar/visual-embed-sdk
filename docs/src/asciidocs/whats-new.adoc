...
* In the REST API SDK [beta blueBackground]^BETA^, the classes corresponding to enumerations used for string fields, such as `type` in metadata API, are renamed. If you are using the REST API SDK in your environments, make sure the class names are updated.
