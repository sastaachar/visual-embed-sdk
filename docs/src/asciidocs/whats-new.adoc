...
== 9.2.0.cl, Coming soon

=== Per Org tokens for Trusted authentication
...
Starting from 9.2.0.cl, Org users require Org-specific authentication tokens issued for their user accounts when switching between Orgs. When generating an authentication token via REST API calls, make sure to request a separate token for each Org context.
...
=== Cross filters and contextual menu changes [eaTag eaBackground]#Early Access#

Starting with 9.2.0.cl, ThoughtSpot supports brushing and linking of visualizations on a Liveboard using cross filters. Cross filters allow you to present a coordinated view of a Liveboard by applying filters across all visualizations based on the current selection. The Cross filters feature is turned off by default in 9.2.0.cl. To enable this feature on your instance, contact ThoughtSpot Support.

==== Contextual menu behaviour

If the cross filters feature is enabled, the contextual menu that currently opens on right-clicking a data point on a visualization will appear as a __left-click-triggered__ pop-up menu. This change makes features like *Drill down* more easily discoverable and improves the user experience for those accessing ThoughtSpot on touch devices.
...
For information about the new features and enhancements introduced in Visual Embed SDK version 1.20.0 and 1.21.0, see xref:api-changelog.adoc[Visual Embed changelog].
