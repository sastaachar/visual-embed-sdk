...
|====
||
|[tag greenBackground]#NEW FEATURE# a| +++<h5>App actions for Slack integration +++ 

ThoughtSpot introduces app actions to support seamless integration with third-party business apps such as Slack. Your application users can now connect ThoughtSpot with their Slack workspaces and deliver insights directly to Slack channels.

Users with developer or admin privileges can create an app action for Slack in the Developer portal and add it as a menu action on visualizations and saved answers. On clicking this action, ThoughtSpot users can initiate the Slack integration workflow and send data to their Slack channels without leaving the ThoughtSpot UI.
...
|====
||
|[tag greenBackground]#NEW FEATURE# a|+++<h5> New API endpoints</h5> +++ 
...
These API endpoints are available only if the REST API v2 Playground and SDK feature is enabled on your instance. The REST API v2 Playground and SDK is a limited availability feature and is in beta. 
...
The `init` method now returns an `authPromise` that resolves when a user authentication is completed.
|====
...
You can use any standards-compliant HTTP client or use the Playground to make an API call. If you want to construct your queries and process API responses programmatically, you can download the SDK and client libraries in the programming language of your choice and integrate it with your applications.
