...
==== REST API v2 [beta orangeBackground]^BETA^

[width="100%" cols="1,4"]
|====
||
|[tag greenBackground]#NEW ENDPOINTS#  a| The following REST API v2[beta orangeBackground]^BETA^ endpoints will be available soon on your instance:
...
* The HTTP request method for the following REST v2 API [beta orangeBackground]^BETA^ endpoints will be changed to `PUT`.
...
* In the REST API SDK [beta orangeBackground]^BETA^, the string fields with enumerations, such as the `type` field in metadata API, are  changed class name.  If you are using the REST API SDK in your environments, make sure the class names are updated. 
|[tag redBackground]#REMOVED# a| The `ownedBy` attribute will be removed from the  `/tspublic/rest/v2/metadata/header/search` API endpoint.
|[tag orangeBackground]#MODIFIED# a|
* The `x-requested-by` header is not mandatory for REST v2 API [beta orangeBackground]^BETA^ calls.
* The `createdBy` attribute in the `/tspublic/rest/v2/metadata/header/search` API will be renamed to `author`.
* Some attributes with the `boolean` data type in the API request parameters will be changed to the `string` data type. However, the existing values assigned to these attributes will persist after an upgrade.
...
ThoughtSpot introduces app actions[beta orangeBackground]^BETA^ to support seamless integration with third-party business apps such as Slack. Your application users can now connect ThoughtSpot with their Slack workspaces and deliver insights directly to Slack channels.

Users with developer or admin privileges can create an app action for Slack[beta orangeBackground]^BETA^ in the Developer portal and add it as a menu action on visualizations and saved answers. On clicking this action, ThoughtSpot users can initiate the Slack integration workflow and send data to their Slack channels without leaving the ThoughtSpot UI.
...
==== Visual Embed SDK 1.10.0

The Visual Embed SDK version 1.10.0 with the `AddRemoveColumns` event is now available. For more information, see xref:api-changelog.adoc[Visual Embed changelog].
...
Starting from 8.2.0.cl release, the xref:rest-api-v2.adoc[REST API Playground and SDK] [beta orangeBackground]^BETA^ feature is enabled by default on ThoughtSpot instances.
...
.REST API Playground and SDK [beta orangeBackground]^BETA^
