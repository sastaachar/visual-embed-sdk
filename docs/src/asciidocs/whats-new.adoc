...
The Visual Embed SDK version 1.10.0 with the `AddRemoveColumns` event will be available soon. For more information, see xref:api-changelog.adoc[Visual Embed changelog].
...
The xref:rest-api-v2.adoc[REST API Playground and SDK] [beta orangeBackground]^BETA^ feature will be enabled by default on ThoughtSpot instances in the upcoming release.

The upcoming release also introduces the following new features:
...
////
SDK for .NET clients::
ThoughtSpot provides the .NET SDK to allow developers to interact with REST APIs from .NET applications. You can download the SDK from NuGet Package Manager and install it using the NuGet GUI.
