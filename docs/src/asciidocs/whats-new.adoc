...
The version control API endpoints now allow you to generate a GUID mapping file that records the GUIDs of the TML objects. The GUID mapping file is essential when your deployment environments have different object GUIDs for the same TML representation. It maps the GUIDs of the TML objects deployed on a production environment to the object GUIDs on the source environment.
...
On multi-tenant clusters with Orgs, developers can now add a list of CORS hosts at the Org level.
////
For more information, see xref:security-settings.adoc#cors-hosts[Security Settings].
////
...
For information about the new features and enhancements introduced in Visual Embed SDK version 1.22.0, see xref:api-changelog.adoc[Visual Embed changelog].
