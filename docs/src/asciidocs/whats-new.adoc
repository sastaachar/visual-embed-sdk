...
=== Cross-filters and change in contextual menu behavior
...
After this feature is enabled on your instance, if your deployment uses `EmbedEvent.VizPointClick` to listen to click events on an embedded Liveboard visualization, the embedded object will emit the `VizPointClick` event when a user clicks a contextual menu action such as *Drill down*, *Filter*, and so on.
...
=== REST API v2 endpoints and Playground

Starting with 9.0.0.cl, the REST API v2[beta betaBackground]^Beta^ API endpoints are deprecated and removed from the REST API v2 Playground. The REST API v2.0 API Playground will include a set of new endpoints that will be generally available (GA) on all ThoughtSpot instances.

The REST API v2.0 endpoints introduce several improvements to the request and response structure and let you perform more tasks in a single API call. For example, you can create a new user, map the user to groups and Orgs, set home Liveboards for the user, and assign privileges in a single API request.

[IMPORTANT]
====
* All REST API v2 [beta betaBackground]^Beta^ endpoints are deprecated. For more information, see xref:deprecated-features.adoc#_deprecation_of_rest_v2_api[Deprecation announcements].
* No new endpoint collections or resources will be introduced in the REST API v2 [beta betaBackground]^Beta^ framework.
* The new REST v2.0 API endpoints are not an extension of the REST API v2 [beta betaBackground]^Beta^ endpoints. The resource categories, base path, and endpoint URIs are not the same as those of the REST API v2 [beta betaBackground]^Beta^ version.
* Some API operations for data connections, Liveboard schedules, and Orgs are not available in the initial release.
...
[%collapsible]
====
Bug fixes and new improvements in embed user experience. +
...
[%collapsible]
====
The REST v2 API [beta betaBackground]^Beta^ feature will be deprecated in 8.10.0.cl and replaced with the new REST v2.0 API endpoints. For more information, see xref:deprecated-features.adoc[Deprecation announcements].
====
...
|====
||
|[tag greenBackground]#NEW ENDPOINTS#  a| This release introduces the following new REST v2 [beta betaBackground]^Beta^ endpoints: +
...
* The `x-requested-by` header is not mandatory in REST v2 [beta betaBackground]^Beta^ API calls.
