...
=== Per Org tokens for Trusted authentication [tag redBackground]#BREAKING CHANGE#

Org users authenticating to ThoughtSpot via Trusted authentication method will require Org-specific tokens to switch between Orgs. After your ThoughtSpot instance is upgraded to the new release version, your Org users will not be able to switch to another Org if they don't have a token to access that Org. When generating an authentication token on behalf of a user via `POST /tspublic/v1/session/auth/token` REST API call, the cluster administrator or Org administrator must make sure to request a token for each Org to which the user belongs.
...
For information about the new features and enhancements introduced in the Visual Embed SDK, see xref:api-changelog.adoc[Visual Embed changelog].
...
The upcoming release includes bug fixes and improvements to the Search bar embed experience, new Liveboard experience, and REST API v2.0 Playground experience.
