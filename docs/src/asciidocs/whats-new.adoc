...
|====
||
|[tag greenBackground]#NEW ENDPOINTS#  a| The following REST API v2[beta blueBackground]^BETA^ endpoints will be available soon on your instance:
...
* In the REST API SDK [beta blueBackground]^BETA^, the classes corresponding to enumerations used for string fields, such as the `type` field in metadata API, have been renamed. If you are using the REST API SDK in your environments, make sure the class names are updated. 
...
* The attributes with the `boolean` data type in the API endpoints will be changed to `string` data type. If a boolean attribute was set as `true` in your existing setup, the value will be changed to `"true"`. 
|====
...
The Visual Embed SDK version 1.10.x is now available. For information about the new features and enhancements introduced in these SDK versions, see xref:api-changelog.adoc[Visual Embed changelog].
