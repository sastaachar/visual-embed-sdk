...
To provide access to ThoughtSpot content or define role-based privileges, you need to add users and groups in ThoughtSpot. You can add link:https://docs.thoughtspot.com/latest/admin/users-groups/add-user.html[users,  window=_blank] and link:https://docs.thoughtspot.com/latest/admin/users-groups/add-group.html[user groups] either in the UI or by using the xref:rest-api-reference.adoc[REST API services].
