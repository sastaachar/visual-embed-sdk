...
== Version 1.17.0, Coming soon

The new version of the SDK introduces several new features and enhancements
[width="100%" cols="1,4"]
|====
|[tag orangeBackground]#MODIFIED# a|The upcoming version of the SDK introduces the following `AuthType` enum changes. +

* `AuthType.SAML` is renamed as `AuthType.SAMLRedirect` +
* `AuthType.OIDC` is renamed as `AuthType.OIDCRedirect` +
* `AuthType.AuthServer` is renamed to `AuthType.TrustedAuth` +
This enhancement does not introduce any breaking changes in your current implementation.
...
|[tag greenBackground]#NEW FEATURE# a| The new version of the SDK introduces the `customizations` property, which allows you to specify a custom CSS URL and define style customization variables in the `init` function.
...
The `enableVizTransformations` attribute in the `LiveboardEmbed` package is deprecated.
|*Other changes*| Bug fixes and improvements to the new Liveboard experience.
|====
...
This enhancement does not introduce any breaking changes in your current implementation.
|[tag greenBackground]#NEW FEATURE# a|The upcoming version of the SDK introduces `AuthType.EmbeddedSSO` for embedded SSO authentication. This authentication method allows you to use SAML or OIDC authentication and redirect users to the IdP within the embedded iFrame.
For more information, see xref:embed-authentication.adoc[Authentication].
...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE#|Ability to configure `redirectPath` on the origin when using the SAML `authType`.
|====
...
This feature is applicable to token-based authentication, that is, when the `AuthType` is set as `AuthServer` in the SDK.
