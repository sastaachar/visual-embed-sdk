...
== Version 1.11.0, Coming soon

[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE#  a| The upcoming version of SDK will include the following new events
...
|[tag greenBackground]#NEW FEATURE#  a| The new version of SDK will include the `showAlerts` attribute, using which you can show or hide alerts and error messages in the embedded view.

|[tag greenBackground]#NEW FEATURE# a| The `Action.CreateMonitor` enumeration will be available in the SDK for emebedded ThoughtSpot environments on which the Monitor feature is enabled.  
...
[tag greenBackground]#MODIFIED#| When login fails, the user will be presented with a `not logged in` message. This message can now be configured using the `loginFailedMessage` attribute.
|====
...
* The `init` method now returns an event emitter which can be used to listen to `AuthStatus` events such as login failure or success.
|====
