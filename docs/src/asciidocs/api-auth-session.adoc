...
== Sign in to ThoughtSpot (Basic authentication)
...
[#sessionCookies]
== Set session cookies for subsequent API calls
...
You can make a test API call to verify if your token is valid and allows access to ThoughtSpot objects. For example, you can send a `GET` request `/tspublic/v1/metadata/listvizheaders` to fetch visualization headers from a Liveboard.
