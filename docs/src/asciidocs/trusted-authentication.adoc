...
Trusted authentication requires you to have an authenticator server or service, which can securely authenticate the users of the embedding application. You **must** implement secure authentication within the process that requests the login token from the authenticator server.
...
The security token, also referred to as the authentication token, allows users to access the requested object.
...
[NOTE]
====
You can also use the `tscli tokenauthentication enable` command to enable trusted authentication. For more information about tscli commands, see link:https://docs.thoughtspot.com.netlify.app/software/latest/tscli-command-ref.html[tscli command reference, window=_blank].
====
...
[NOTE]
====
You can also use the `tscli tokenauthentication disable` command to disable trusted authentication. For more information about `tscli` commands, see link:https://docs.thoughtspot.com/software/latest/tscli-command-ref.html[tscli command reference, window=_blank].
====