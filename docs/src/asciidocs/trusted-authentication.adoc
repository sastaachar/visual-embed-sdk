...
* When a new token is generated using the same secret key, the old token expires after a few minutes.
* When you generate a new token using the new secret key, the old token becomes invalid immediately.
