...
Note the following issues and limitations before you enable new Liveboard experience on your embedded instance:

* Custom actions, particularly __local__ actions, may not show on visualization pages or work as expected.
* The `locale` setting translates only some parts of the embedded Liveboard such as the menu action labels.
* The *Upload TML* and *Manage schedule* menu actions on embedded Liveboards show an error.
