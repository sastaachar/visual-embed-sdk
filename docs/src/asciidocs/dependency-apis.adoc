...
|====
|Query parameter| Description
|`id`|__Array of strings__. The GUIDs of data objects such as worksheets, tables, or views.
|====
...
If a pinboard is set as a default pinboard for a user group, the `/tspublic/v1/dependency/pinboard` endpoint will return the  user group as a dependent object for that pinboard. This API endpoint does not return other dependent data objects such as tables, views, and worksheets.
...
|====
|Query parameter|Description
|`id`|__Array of strings__. A JSON array of pinboard GUIDs.
|====
